...
from utils import load_var_data,VARDataset
import numpy as np
...
print(args)
input_size = 128
X_train, Y_train, X_test, Y_test = load_var_data(args.data)
...
def evaluate(X_data, Y_data, name='Eval'):
    ...
    total_loss = 0.0
    count = 0
    eval_set = VARDataset(X=X_data, Y=Y_data)
...
def train(ep):
    ...
    train_set = VARDataset(X=X_train, Y=Y_train)
    loader_args = dict(batch_size=args.batchsize, pin_memory=True)
    ...
    for batch in train_loader:
        ...
        optimizer.zero_grad()
        output = model(x).squeeze(-1)
        reconstruction_loss = nn.MSELoss()(output, y)
        loss = kl_divergence_loss(y,output)+reconstruction_loss
        total_loss += loss.item() * data_line.shape[0]
        count += data_line.shape[0]
