...
class TemporalBlock(nn.Module):
    def __init__(self, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2):
        ...
        self.conv1 = weight_norm(nn.Conv2d(n_inputs, n_outputs, kernel_size, stride=stride, padding=padding, dilation=dilation))
        self.chomp1 = Chomp1d(padding)
        self.relu1 = nn.ReLU()
        self.dropout1 = nn.Dropout(dropout)

        self.conv2 = weight_norm(nn.Conv2d(n_outputs, n_outputs, kernel_size,stride=stride, padding=padding, dilation=dilation))
...
class ConvEncoder(nn.Module):
    def __init__(self, input_size):
        ...
        layers = []
        self.layer_1 = weight_norm(nn.Conv2d(input_size, 1, 3, stride=1))
        self.layer_2 = nn.CELU()
        self.layer_3 = nn.MaxPool1d(2)
        self.layer_4 = weight_norm(nn.Conv2d(1,1,3,stride=1))
...
class ConvDecoder(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.layer_1 = weight_norm(nn.ConvTranspose2d(self.input_size,self.output_size//2,3,stride=1))
        # layers.append(nn.ReLU())
        self.layer_2 = nn.CELU()
        self.layer_3 = nn.Upsample(scale_factor=2)
        self.layer_4 = weight_norm(nn.ConvTranspose2d(self.output_size//2, self.output_size, 3, stride=1))
